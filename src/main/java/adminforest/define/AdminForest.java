package adminforest.define;

public class AdminForest {

	// common
	public static String CONFIG_TXT = "config/config.txt";
	public static String UPLOAD_DIR = "upload.temp.dir";
	public static String KEY_DEFAUT_PAGE = "defaultPage";
	
//	public static String DEFAUT_PAGE_FILE = "config/defaultPage.txt";
	
	public static String LIST_FILE_VO="listFileVo";
//	public static String KEY_LANDSCAPE_IMAGES = "landscapeImages";
	public static String KEY_LANDSCAPE_TYPE = "landscapeType";
	
	public static String DB_DRIVER="db.driver";
	public static String DB_URL="db.url";
	public static String DB_USER="db.user";
	public static String DB_PW="db.pw";
	
	// domain
	public static String UPLOAD_LANDSCAPE = "/upload-files/image-landscape";
	public static String UPLOAD_FOOTER= "/upload-files/image-footer";
	public static String UPLOAD_PORTRAIT = "/upload-files/image-portrait";
	
	public static String LiST_PORTRAIT_VO = "listPortraitVo";
	
	public static String VALUE_IMAGE_FIXED= "image-fixed";
	public static String VALUE_IMAGES_ROLLING="images-rolling";
	
}
